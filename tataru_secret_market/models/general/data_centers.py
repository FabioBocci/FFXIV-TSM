...
class DataCenters(models.Model):
    ...
    def sync_datacenter(self, starting):
        ...
        for dc in data:
            data_center = self.env['tataru_secret_market.data_centers'].search([('name', '=', dc['name'])])

            if not data_center:
                self.env['tataru_secret_market.data_centers'].create({
                    'name': dc['name'],
                    'region': dc['region']
                })

            for world in dc['worlds']:
                world_id = self.env['tataru_secret_market.worlds'].search([('unique_id', '=', int(world))])
