...
class ItemRecipe(models.Model):
    ...
    def sync_recipes(self, item_id):
        ...
        try:
            res = requests.get(f"https://xivapi.com/Item/{item_id.unique_id}?columns=Recipes")
        ...
        for recipe in data["Recipes"]:
            ...
            item_id.write({
                'craftable': True,
                "crafting_recipe_id": recipe_id.id
            })
            try:
                res = requests.get(f"https://xivapi.com/recipe/{recipe_id.unique_id}?columns=Name,ID," +
                ...
                raise Exception(err)
                                # AmountIngredient0
                                # AmountIngredient
            ammounts_keys = [f"AmountIngredient{i}" for i in range(self.MAX_SIZE_OF_INGREDIENTS)]
            item_ids_keys = [f"ItemIngredient{i}" for i in range(self.MAX_SIZE_OF_INGREDIENTS)]
            data = res.json()
            for i in range(self.MAX_SIZE_OF_INGREDIENTS):
                if data[ammounts_keys[i]] is None or int(data[ammounts_keys[i]]) == 0:
                    continue

                ingredient_id = self.env['tataru_secret_market.items'].search([('unique_id', '=', int(data[item_ids_keys[i]]["ID"]))])

                if not ingredient_id:
                    # TODO - log error
                    continue

                find_ing = recipe_id.ingredients_ids.filtered(lambda x: x.item_id.id == ingredient_id.id)

                if find_ing:
                    find_ing.write({
                        'quantity': data[ammounts_keys[i]]
                    })
                    continue
                else:
