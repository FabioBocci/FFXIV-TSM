    ...
    def _compute_price_to_sell(self):
        ...
        for record in self:
            ...
            record.price_to_sell = sum(
                record.item_id.transactions_ids.filtered(lambda x: x.high_quality == use_only_hq or not use_only_hq).mapped('price')
    ...
    def _compute_price_to_craft(self):
        ...
        for record in self:
            ...
            for ingredient in ingredients:
                availability = ingredient.item_id.availability_ids.filtered(lambda x: x.price > 0).sorted('price', reverse=True)
